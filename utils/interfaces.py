...
class Interface(object):
    ...
    def vector(self) -> np.array:
        return np.array([self.relevance])
        # return np.array([1])
...
class SwitchInterface(Interface):
    def __init__(
        self, relevance=0, current_value=0, id=1, on_value=1, off_value=0
    ) -> None:
    ...
    # def vector(self) -> np.array:
    #     return np.array(
    #         [self.relevance, self.current_value, self.on_value, self.off_value]
    #     )
...
class StateInterface(Interface):
    def __init__(
        self, relevance=0, current_value=0, id=4, state=0, interpreter=0
    ) -> None:
        super().__init__(relevance, current_value, id=id)
        self.state = state
    ...
    # def vector(self) -> np.array:
    #     return np.array(
    #         [self.relevance, self.current_value, self.state, self.interpreter]
    #     )
