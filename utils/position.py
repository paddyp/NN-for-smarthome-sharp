...
from fixture import SMART_DEVICES, ROOM_SIZE
import numpy as np 
...
def calc_direction_vector(smart_device: tuple, position_point: np.array, add_noise=True):
    _, smart_device_value = smart_device
    smart_device_point = smart_device_value["position"]
    # add noise to smart_device_point
    if add_noise: 
        noise = np.random.normal(0, smart_device_value["noise"]/2, smart_device_point.shape)
...
def calc_angle_between_position_and_device(direction_vector: np.array, device_position: np.array, position_in_room: np.array): 
    direct_direction_vector = device_position - position_in_room
    return np.arccos(np.clip(np.dot(unit_vector(direct_direction_vector), unit_vector(direction_vector)), -1.0, 1.0))
