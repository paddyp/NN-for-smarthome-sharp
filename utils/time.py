...
class TimeFaker:
    ...
    def get_random_datetime_as_list(self):
        return [
            self.random_datetime.weekday() / 7,
            self.random_datetime.hour / 24,
            self.random_datetime.minute / 60,
        ]
    ...
    def _set_time_range(self, time_range):
        if (
            ...
            and type(time_range[0]) == tuple
            and type(time_range[1])
        ):
            return time_range
